...
const steps = [
    ...
    },
    {
        ...
        icon: <Landmark className="w-16 h-16 text-sage" />,
        action: (
    ...
    },
    {
        ...
        icon: <CheckCircle2 className="w-16 h-16 text-ember" />,
        action: (
            ...
            <DialogContent className="sm:max-w-md text-center flex flex-col items-center py-10 px-6">
                ...
                <div className="flex items-center justify-center gap-2 mt-6 mb-8 w-full max-w-[200px] mx-auto">
                    {steps.map((step, index) => (
                        <React.Fragment key={step.label}>
                            <div className={`w-3 h-3 rounded-full ${index === activeStep ? 'bg-primary' : index < activeStep ? 'bg-primary/50' : 'bg-gray-200'}`} />
                            {index < steps.length - 1 && (
                                <div className={`flex-1 h-0.5 ${index < activeStep ? 'bg-primary/50' : 'bg-gray-200'}`} />
                            )}
                        </React.Fragment>
                    ))}
                </div>
                ...
                <DialogFooter className="w-full sm:justify-center flex mt-2">
                    {activeStep === steps.length - 1 ? (
                        <Button className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold" onClick={handleClose}>
                            Get Started
                        </Button>
                    ) : (
                        <Button className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold" onClick={handleNext}>
                            Next
                        </Button>
                    )}
                </DialogFooter>
